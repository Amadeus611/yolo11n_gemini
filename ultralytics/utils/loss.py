...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        # 支持 DS-NIM 损失切换
        self.use_dsnim = h.get('use_dsnim', False) if isinstance(h, dict) else getattr(h, 'use_dsnim', False)
        if self.use_dsnim:
...
# ============================================================================
# DS-NIM Loss: 航拍专属动态协同归一化 Wasserstein 与内最小点距离损失
# ============================================================================


def nwd_loss(pred_bboxes: torch.Tensor, target_bboxes: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """归一化 Wasserstein 距离损失 (Normalized Wasserstein Distance Loss)。

    将边界框建模为二维高斯分布，利用最优传输理论计算距离。
    即使预测框与真实框完全不相交（IoU=0），也能提供平滑连续的梯度方向。

    Args:
        pred_bboxes: 预测框 (x1, y1, x2, y2)，形状 (N, 4)。
        target_bboxes: 真实框 (x1, y1, x2, y2)，形状 (N, 4)。
        eps: 数值稳定性常数。
    ...
    pred_w = (pred_bboxes[:, 2] - pred_bboxes[:, 0]).clamp(min=eps)
    pred_h = (pred_bboxes[:, 3] - pred_bboxes[:, 1]).clamp(min=eps)

    gt_cx = (target_bboxes[:, 0] + target_bboxes[:, 2]) / 2
    gt_cy = (target_bboxes[:, 1] + target_bboxes[:, 3]) / 2
    gt_w = (target_bboxes[:, 2] - target_bboxes[:, 0]).clamp(min=eps)
    gt_h = (target_bboxes[:, 3] - target_bboxes[:, 1]).clamp(min=eps)

    # 中心距离的平方
    center_dist2 = (pred_cx - gt_cx) ** 2 + (pred_cy - gt_cy) ** 2

    # 协方差矩阵的 Wasserstein 距离
    # 对于轴对齐的高斯分布，Wasserstein 距离有闭式解
    # sigma_pred = diag(w/2, h/2), sigma_gt = diag(w/2, h/2)
    # W^2 = ||mu1-mu2||^2 + trace(S1 + S2 - 2*(S1^{1/2}*S2*S1^{1/2})^{1/2})
    # 对于对角协方差，简化为:
    pred_w_half = pred_w / 2
    pred_h_half = pred_h / 2
    gt_w_half = gt_w / 2
    gt_h_half = gt_h / 2

    wh_term = (pred_w_half - gt_w_half) ** 2 + (pred_h_half - gt_h_half) ** 2

    # 归一化因子: 将距离归一化到 [0, 1] 区间
    # 使用框的对角线长度作为归一化尺度
    pred_diag2 = pred_w ** 2 + pred_h ** 2
    gt_diag2 = gt_w ** 2 + gt_h ** 2
    norm_factor = torch.sqrt(pred_diag2 * gt_diag2) + eps

    # NWD = exp(-W^2 / norm_factor)
    wasserstein2 = center_dist2 + wh_term
    nwd = torch.exp(-torch.sqrt(wasserstein2 + eps) / (norm_factor.sqrt() + eps))

    # 返回 1 - NWD 作为损失（越小越好）
    return (1 - nwd).unsqueeze(-1)
...
def inner_mpdiou(
    ...
    """内最小点距离 IoU 损失 (Inner-MPDIoU Loss)。

    结合 Inner 缩放机制和 MPDIoU（Minimum Point Distance IoU），
    通过显式计算边界框左上角和右下角的几何最小点距离，
    增强在高重叠度状态下对目标边界微小形变的敏感度。

    Args:
        pred_bboxes: 预测框 (x1, y1, x2, y2)，形状 (N, 4)。
        target_bboxes: 真实框 (x1, y1, x2, y2)，形状 (N, 4)。
        inner_ratio: 内缩比例因子，控制辅助框的大小。
        eps: 数值稳定性常数。

    Returns:
        Inner-MPDIoU 损失值，形状 (N, 1)。
    """
    # 提取坐标
    ...
    inner_pw = pw * (1 - inner_ratio)
    inner_ph = ph * (1 - inner_ratio)
    inner_tw = tw * (1 - inner_ratio)
    inner_th = th * (1 - inner_ratio)
    ...
    inner_pred_area = inner_pw * inner_ph
    inner_gt_area = inner_tw * inner_th
    union_area = inner_pred_area + inner_gt_area - inter_area + eps
    inner_iou = inter_area / union_area

    # MPDIoU: 最小点距离
    # 计算左上角和右下角的距离
    d1_sq = (px1 - tx1) ** 2 + (py1 - ty1) ** 2  # 左上角距离
    d2_sq = (px2 - tx2) ** 2 + (py2 - ty2) ** 2  # 右下角距离

    # 归一化: 使用图像对角线（这里用框的对角线近似）
    diag_sq = ((torch.max(px2, tx2) - torch.min(px1, tx1)) ** 2 +
               (torch.max(py2, ty2) - torch.min(py1, ty1)) ** 2) + eps

    mpd = (d1_sq + d2_sq) / diag_sq

    # Inner-MPDIoU = Inner_IoU - MPD
    # 损失 = 1 - Inner-MPDIoU
    inner_mpdiou_val = inner_iou - mpd.clamp(max=1.0)
    return (1 - inner_mpdiou_val).clamp(min=0)


class BboxLossDSNIM(nn.Module):
    """DS-NIM 损失封装器。

    动态协同归一化 Wasserstein 与内最小点距离损失。
    在训练过程中，根据 epoch 和目标尺度自适应地在 NWD 和 Inner-MPDIoU 之间切换。

    Args:
        reg_max (int): DFL 正则化最大值。
        use_dsnim (bool): 是否启用 DS-NIM 损失。
        nwd_weight (float): NWD 损失的权重。
        inner_mpdiou_weight (float): Inner-MPDIoU 损失的权重。
        inner_ratio (float): Inner 缩放比例。
        alpha_start (float): 训练初期 NWD 的主导权重。
        alpha_end (float): 训练后期 MPDIoU 的主导权重。
    """
    ...
    def __init__(
        ...
        self.total_epochs = 300  # 默认值，会在训练时更新
    ...
    def _get_alpha(self) -> float:
        ...
        alpha 从 alpha_start 线性变化到 alpha_end。
        当前公式中，alpha 越小越依赖 NWD，alpha 越大越依赖 Inner-MPDIoU。
        """
    ...
    def forward(
        ...
        Returns:
            (iou_loss, dfl_loss) 元组。
        """
        weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)

        if self.use_dsnim and fg_mask.sum():
            # DS-NIM: 动态协同损失
            ...
            # Inner-MPDIoU 损失（精确边界回归）
            loss_impd = inner_mpdiou(pred_fg, target_fg, inner_ratio=self.inner_ratio)

            # 数值稳定性检查
            loss_nwd = loss_nwd.clamp(min=0, max=2)
