...
# ============================================================================
# GL-SIM: 全局-局部协同交互机制 (Global-Local Synergistic Interaction Mechanism)
# ============================================================================


class GCAM(nn.Module):
    """全局上下文感知掩码器 (Global Context-Aware Masker).

    通过多尺度自适应池化和轻量级组卷积提取全局低频语义信息，
    输出归一化至 [0, 1] 的全局空间置信度掩码。
    用于替换 YOLO11n 中的 C2PSA 模块，植入 Backbone 最深层（SPPF 之后）。
    掩码通过 self.mask 存储，供下游 DCRN 模块使用。

    Args:
        c1 (int): 输入通道数。
        c2 (int): 输出通道数（通常等于 c1）。
        pool_sizes (tuple): 自适应池化的输出尺寸列表。
    ...
    def __init__(self, c1: int, c2: int, pool_sizes: tuple = (2, 3, 5), reduction: int = 16):
        ...
        # 多尺度自适应池化分支（pool_size >= 2 避免 BatchNorm 在 1x1 上报错）
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(c_mid * n_pools, c_mid, 1, bias=False),
            nn.BatchNorm2d(c_mid),
            nn.SiLU(),
            nn.Conv2d(c_mid, 1, 1, bias=False),
            nn.Sigmoid(),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            经全局掩码调制后的特征图 (B, C, H, W)。
        """
        B, C, H, W = x.shape

        # 多尺度池化 → 上采样到原尺寸 → 拼接
        pooled = []
        for pool_branch in self.pools:
            feat = pool_branch(x)  # (B, c_mid, ps, ps)
...
class LHDE(nn.Module):
    """局部高频细粒度提取器 (Local High-Frequency Detail Extractor).

    双分支架构：主分支保留原始特征，侧分支使用可学习的拉普拉斯风格
    高频滤波器捕捉剧烈的空间变化率。
    战略性部署于 Neck 网络的浅层高分辨率特征流入口（P3 层）。

    Args:
        c1 (int): 输入通道数。
        c2 (int): 输出通道数（通常等于 c1）。
        reduction (int): 侧分支通道压缩比。
    """

    def __init__(self, c1: int, c2: int, reduction: int = 4):
        super().__init__()
        c_side = max(c1 // reduction, 8)

        # 侧分支: 高频滤波器
        # 使用 3x3 卷积初始化为拉普拉斯核风格，捕捉边缘和纹理
        self.hf_conv = nn.Conv2d(c1, c_side, 3, 1, 1, bias=False)
        self.hf_bn = nn.BatchNorm2d(c_side)
        self.hf_act = nn.SiLU()

        # 侧分支: 特征增强
        self.hf_enhance = nn.Sequential(
            nn.Conv2d(c_side, c_side, 3, 1, 1, bias=False),
            nn.BatchNorm2d(c_side),
            nn.SiLU(),
        )

        # 主分支: 轻量通道压缩保持原始信息
        self.main_conv = Conv(c1, c2 - c_side, 1, 1)

        # 输出融合
        self.fuse = Conv(c2, c2, 1, 1)

        self._init_hf_weights()

    def _init_hf_weights(self):
        """初始化高频滤波器权重为拉普拉斯风格。"""
        with torch.no_grad():
            ...
            # 对每个输入通道的每个输出通道都使用相同的拉普拉斯核初始化
            for i in range(self.hf_conv.weight.shape[0]):
                for j in range(self.hf_conv.weight.shape[1]):
                    self.hf_conv.weight[i, j] = laplacian
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            融合后的特征图 (B, C, H, W)，包含原始信息和高频细节。
        """
        # 主分支: 保留原始空间信息
        main = self.main_conv(x)

        # 侧分支: 提取高频边缘和纹理
        hf = self.hf_act(self.hf_bn(self.hf_conv(x)))
        hf = self.hf_enhance(hf)

        # 拼接并融合
        return self.fuse(torch.cat([main, hf], dim=1))


class DCRN(nn.Module):
    """动态协同路由节点 (Dynamic Collaborative Routing Node).
...
# ============================================================================
# RAC-Net: 冗余感知协作卷积网络 (Redundancy-Aware Collaborative Convolution Network)
# ============================================================================


class PConv(nn.Module):
    """部分卷积 (Partial Convolution) 模块。

    仅选取部分代表性通道进行常规空间卷积，其余通道直接恒等映射。
    大幅削减浮点运算量并缓解内存访问带宽压力。

    Args:
        c1 (int): 输入通道数。
        c2 (int): 输出通道数。
        n_div (int): 通道划分比例（处理 1/n_div 的通道）。
        k (int): 卷积核大小。
    """

    def __init__(self, c1: int, c2: int, n_div: int = 4, k: int = 3):
        super().__init__()
        self.c_partial = c1 // n_div
        # 仅对部分通道进行卷积
        self.partial_conv = nn.Conv2d(
            self.c_partial, self.c_partial, k, 1, k // 2, bias=False
        )
        self.bn = nn.BatchNorm2d(self.c_partial)
        self.act = nn.SiLU()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        # 划分通道
        x_partial = x[:, :self.c_partial, :, :]
        x_identity = x[:, self.c_partial:, :, :]

        # 对部分通道执行卷积
        x_partial = self.act(self.bn(self.partial_conv(x_partial)))

        # 拼接: 处理后的通道 + 恒等通道
        return torch.cat([x_partial, x_identity], dim=1)


class CCFR(nn.Module):
    """跨通道廉价特征幻影重组 (Cross-Channel Cheap Feature Reuse)。

    接收 PConv 处理的活跃通道，通过低成本线性操作（深度可分离卷积）
    快速生成幻影特征图，与恒等映射通道进行跨通道融合。

    Args:
        c1 (int): 输入通道数。
        c2 (int): 输出通道数。
        n_div (int): 与 PConv 一致的通道划分比例。
    """

    def __init__(self, c1: int, c2: int, n_div: int = 4):
        super().__init__()
        self.c_partial = c1 // n_div
        ...
        self.cheap_conv = nn.Sequential(
            # 分组卷积: 极低成本
            nn.Conv2d(self.c_partial, self.c_partial, 3, 1, 1,
                      groups=self.c_partial, bias=False),
            nn.BatchNorm2d(self.c_partial),
            nn.SiLU(),
            # 1x1 点卷积: 跨通道信息交换
            nn.Conv2d(self.c_partial, self.c_partial, 1, 1, 0, bias=False),
            nn.BatchNorm2d(self.c_partial),
            nn.SiLU(),
        )

        # 融合层: 幻影特征 + 恒等通道 → 输出
        self.fuse = Conv(c1, c2, 1, 1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """前向传播。

        Args:
            x: 输入特征图 (B, C, H, W)，来自 PConv 的输出。

        Returns:
            跨通道融合后的特征图 (B, C2, H, W)。
        """
        # 提取活跃通道和恒等通道
        x_active = x[:, :self.c_partial, :, :]
        x_identity = x[:, self.c_partial:, :, :]

        # 从活跃通道生成幻影特征
        ghost = self.cheap_conv(x_active)

        # 跨通道融合: 幻影特征替换部分恒等通道
        # 将 ghost 与 identity 的前 c_partial 通道融合
        fused_active = x_active + ghost  # 残差融合
        x_out = torch.cat([fused_active, x_identity], dim=1)

        return self.fuse(x_out)


class C3k2_PConv(C2f):
    """基于 PConv + CCFR 的轻量化 C3k2 模块。

    将 C3k2 内部的 Bottleneck 替换为 PConv + CCFR 协同结构，
    实现参数量和计算量的极致压缩。
    ...
    def __init__(
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        # 用 PConv+CCFR 替换原始的 Bottleneck
        self.m = nn.ModuleList(
            nn.Sequential(
                PConv(self.c, self.c, n_div=n_div),
                CCFR(self.c, self.c, n_div=n_div),
            )
            for _ in range(n)
        )
