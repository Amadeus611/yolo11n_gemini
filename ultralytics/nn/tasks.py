...
class BaseModel(torch.nn.Module):
    ...
    def loss(self, batch, preds=None):
        ...
        # DS-NIM 损失: 更新训练进度
        if hasattr(self.criterion, 'bbox_loss') and hasattr(self.criterion.bbox_loss, 'set_epoch'):
            epoch = getattr(self, 'epoch', 0)
            total_epochs = getattr(self, 'total_epochs', getattr(self.args, 'epochs', 300))
            self.criterion.bbox_loss.set_epoch(epoch, total_epochs)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is C3k2:  # for M/L/X sizes
                ...
                if scale in "mlx":
                    args[3] = True
            if m is C3k2_PConv:  # RAC-Net 轻量化模块
                legacy = False
            if m is A2C2f:
                legacy = False
